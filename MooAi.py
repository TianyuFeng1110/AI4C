...
model = joblib.load("./model/xgboost/model_lst_day_c_bk_str_we.pkl")
...
class MyProblem(Problem):
    ...
    def __init__(self):
        super().__init__(
            ...
            xl=np.array([0 ,50 ,0 ,6  ,5  ,0.6 ,0.7  ,0.2 ,80 , 0 ,250 ,80  ,12 ,150 ,80  ,10 ,5  ,2 ,0.6 ,5.0 ,80  ,0.8  ,3.3 ,1.12 ,1.7 ,3.4]),  # 变量下界
            xu=np.array([5 ,70 ,5, 15 ,15 ,0.8 ,0.85 ,0.4 ,150, 9 ,400 ,150 ,25 ,300 ,150 ,20 ,10 ,5 ,0.7 ,8.0 ,200 ,1.25 ,5.0 ,1.67 ,2.5 ,5.0])  # 变量上界
        )
    ...
    def _evaluate(self, X, out, *args, **kwargs): # 101,26
        with torch.no_grad():
            ...
            for n in range(X.shape[0]):
                ...
                f1 = temp_pred[:, 0] # 白天平均地表温度
                f2 = walk_pred[:, 1] # 共享单车周末出发量

                arrays.append(np.column_stack([f1, f2])[id_idx])

        out["F"] = np.vstack(arrays)
