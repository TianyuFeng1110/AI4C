...
def excel_predict_light_filter(file_name):
    ...
    labels = df[['light_dnb_', 'lst_day_c', 'lst_night_', 'bk_st_wk', 'bk_st_we', 'bk_ar_we', 'bk_str_wk',
                 'bk_str_we']].values  # 所有的宜居环境变量列作为标签(要预测的y)
...
def get_five_fold_datasets(data_file, random_seed=42):
        ...
        return city_trans_dataset

# 将X拼接到完整的栅格数据中，取代对应预测位置
def get_all_data_from_X(x, idx, features):
    res = features.detach().clone()
    res[idx] = x
    return res