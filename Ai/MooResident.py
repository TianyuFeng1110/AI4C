...
class MyProblem(Problem):
    ...
    def __init__(self):
        super().__init__(
            ...
            xl=np.array([0.50 ,0.05 ,0    ,0.20 ,0.10 ,0.3 ,0.40 ,0.4 ,30, 0 ,30 ,40 ,18 ,50  ,100  ,5 ,3 ,1 ,25 ,1.0 ,0.24 ,0.5 ,2.5 ,0.58 ,2.04 ,5.10]),  # 变量下界
            xu=np.array([0.60 ,0.15 ,0.02, 0.35 ,0.15 ,0.5 ,0.60 ,0.7 ,60, 9 ,80 ,80 ,35 ,100 ,200 ,10 ,5 ,2 ,35 ,2.0 ,0.54 ,0.8 ,3.3 ,0.83 ,3.00 ,7.50])  # 变量上界
        )

        self.model_utils = utils
        self.model_path = "../model/dl/saved_models/model_fold0.pt"
        self.data_file = "../model/dl/shanghai_data_v3.xlsx"
        self.device = self.model_utils.select_device(4)
    ...
    def _evaluate(self, X, out, *args, **kwargs): # 101,26
        with torch.no_grad():
            ...
            for n in range(X.shape[0]):
                ...
                temp = temp_pred[:, 1] # 夜间地表温度
                ride = walk_pred[:, 3] # 共享单车工作日平均路段骑行量

                arrays.append(np.column_stack([temp, ride])[id_idx])

        temp = np.vstack(arrays)[:, 0]  # 夜间地表温度
        ride = np.vstack(arrays)[:, 1]  # 共享单车工作日平均路段骑行量

        # 条件1：夜间地表温度
        QTmax, QTmin = 26, 20  # 夜间最适温度区间为20-26
        a, b = 10000, 1  # ab分别为温度惩罚系数，当温度大于适宜温度区间时过热的惩罚要大于温度小于适宜温度区间过冷的惩罚
...
np.savetxt("./Resident.txt", res.X, fmt="%.6f", delimiter=" ")